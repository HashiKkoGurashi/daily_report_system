...
public class EmployeeService extends ServiceBase {
    ...
    */
    public long countByCode(String code) {
        ...
        long employee_count = em.createNamedQuery(JpaConst.Q_EMP_COUNT_REGISTERED_BY_CODE, Long.class).setParameter(JpaConst.JPQL_PARM_CODE, code).getSingleResult();
    ...
    public List<String> update(EmployeeView ev, String pepper){
        ...
        if(!savedEmp.getCode().equals(savedEmp.getCode())) {
            //社員番号を更新する場合
            //社員番号についてのバリデーションを行う
            validateCode = true;
            //変更後の社員番号を設定する
        ...
        //更新内容についてバリデーションを行う
        List<String> errors = EmployeeValidator.validate(this, ev, validateCode, validatePass);
